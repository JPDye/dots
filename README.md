...
Personal NixOS + home-manager flake. Single source of truth for system config, dotfiles, dev shells, and theming. Works on three hosts:
...
```sh
sudo pacman -S --needed tailscale udisks2
sudo systemctl enable --now tailscaled
sudo tailscale up
```

`tailscaled` needs root and a TUN device, so standalone home-manager cannot own it. Installing `pkgs.tailscale` into the user profile would give the CLI but no daemon.
...
| `nh.nix` | The `nh` nix-helper. `flake = ~/.config/nix`, a plain path rather than `inputs.self.outPath`, so each run re-reads working-tree edits instead of a frozen store path. Also sets `NH_FLAKE` in the nushell env, which nushell does not get otherwise. Auto-cleans generations: keep last 5, plus everything from past 7 days. Enables `nix-output-monitor`. |
