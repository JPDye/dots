...
# Every eww call carries --no-daemonize. Without it, a client that hits a
# transient IPC failure auto-forks a *second* daemon (seen in the wild while
# the real daemon was busy mapping this very window). The rogue steals the
# IPC socket, orphaning the real daemon's full-screen window where no
# `eww close` can ever reach it — an invisible surface eating every click
# until both services are restarted. With the flag the client just errors:
# we die loudly and systemd restarts us into the cleanup path in main.
def reconcile [] {
    let want = (^niri msg --json overview-state | from json | get is_open)
    let have = (^eww --no-daemonize active-windows | lines | any { |w| $w | str starts-with "powermenu" })
    ...
    if $want and not $have {
        ...
        ^eww --no-daemonize update $"pad-x=($cfg.pad_x)" $"pad-y=($cfg.pad_y)" "menu-open=false" "confirm="
        ^eww --no-daemonize open powermenu --screen $cfg.focused
        ^eww --no-daemonize update menu-open=true
        ...
        # and let systemd restart us into the cleanup path in main.
        ^eww --no-daemonize close powermenu
    }
}
...
def main [] {
    ...
    # service restarts us, so cleaning up here — then reconciling once, in
    # case we died mid-toggle with the overview still open — makes that
    # self-healing.
    try { ^eww --no-daemonize close powermenu }
    reconcile
