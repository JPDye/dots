...
let
  # Absolute, wrapGL'd path to the active terminal — niri spawns with its own
  # PATH (not a login shell's), and on Arch the terminal needs the nixGL
  # wrapper that programs.<terminal> does not apply. Identity on NixOS.
  ...
  # is the absolute `term` path above, not a bare name.
  work-layout =
    let
