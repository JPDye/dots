...
let
  ...
  # procps (pgrep), coreutils (cat/seq/sleep), systemd (loginctl) — the bare
  # commands the helpers below call. writeShellScript does no PATH wrapping, so
  # prepend them explicitly (reliable on NixOS, but not guaranteed on Arch's
  # systemd-user PATH).
  lockBinPath = lib.makeBinPath (
    with pkgs;
    [
      procps
      coreutils
