    ...
    lib.mkMerge [
      {
        ...
        systemd.user.services.eww = {
          ...
          Service = {
            ...
            # so without a shell here the media listener never starts (ENOENT).
            Environment = [
              "PATH=${
                lib.makeBinPath [
                  pkgs.eww
        ...
        systemd.user.services.eww-powermenu = {
          Unit = {
            ...
            Wants = [ "eww.service" ];
            # powermenu.nu dies on any eww hiccup by design (--no-daemonize turns
            # IPC failures into loud exits); Restart=always must never give up on
            # it, so disable the start-rate limit.
