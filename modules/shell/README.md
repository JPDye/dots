...
├── fastfetch.nix        system-info banner shown at shell start
├── integrations.nix     programs with nushell hooks (zoxide, atuin, ...)
├── nix-index.nix        nix-index + comma (run uninstalled binaries: `, cmd`)
├── nushell.nix          programs.nushell config + script wiring
├── starship.nix         two-line prompt
├── yazi.nix             terminal file manager (`y`)
└── nushell/
    ├── welcome.nu       runs on shell start
    └── scaffolds.nu     project scaffolders (init-rust, ...)
```

`default.nix` imports the eight `.nix` files. The two `.nu` files are
...
| `init-typst`  | `templates/typst/`  | typst, tinymist, typstyle                                    |
