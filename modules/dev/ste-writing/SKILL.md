...
When the task is to write or rewrite a document, output only the requested text. No preamble, no summary, no closing remarks.
...
For a deterministic check, run the bundled linter (in this skill's directory) on the finished text — the score is violations per 100 words, lower is cleaner:

```
python3 ~/.claude/skills/ste-writing/ste-lint.py draft.md   # or pipe text on stdin
```
