...
let
  ...
  # Global rules (~/.config/opencode/AGENTS.md): the same "STE always on"
  ...
  # ste-writing ruleset (frontmatter stripped) at build time. The skill
  # itself stays invocable as `ste-writing` — opencode discovers it from
  ...
  agentsMd = pkgs.runCommand "opencode-agents-md" { } ''
    substitute ${./claude-user-memory.md} $out \
      --replace-fail '@~/.claude/skills/ste-writing/SKILL.md' \
                     '(inlined below from the ste-writing skill)'
    awk '/^---$/ && c < 2 { c++; next } c == 2' ${./ste-writing/SKILL.md} >> $out
  '';
  ...
  config = lib.mkIf cfg.enable {
    home = {
      ...
      file = {
        ...
        # STE always on for all prose (see agentsMd above).
