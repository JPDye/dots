...
let
  ...
  # Light-variant accent treatment: saturate the dark accent (so it reads as a
  # colour, not a tinted grey, against cream) then blend ~15% toward black for
  # contrast. Applied uniformly, so the light palette keeps the same "one tonal
  # level" structure as `dark` — just punchier than dark accents shown on a
  # light bg, without the tonal scatter of the hand-tuned `*Dark` shades.
  lightAccent = c: mix 0.15 (saturate 30 c) "000000";
  ...
  # Light variant: fg/bg ramps swapped, and accents derived from the dark
  # accents via `lightAccent` (saturate + mild darken) so they keep their hue
  # and the palette's uniform tone but read as punchy colours on the cream bg
  # instead of washing out. The semantic roles are re-derived here (in a `rec`)
  # because the dark set bound them to dark's accents, so `dark // { red = …; }`
  # alone would not update `border`/`accent`/etc. (*Vivid shades are still
  # dark-derived — a visual-tuning follow-up, not load-bearing.)
  light = dark // rec {
    bg0 = "fbf1c7";
    bg1 = "ebdbb2";
    bg2 = "d5c4a1";
    ...
    red = lightAccent dark.red;
    green = lightAccent dark.green;
    yellow = lightAccent dark.yellow;
    orange = lightAccent dark.orange;
    blue = lightAccent dark.blue;
    pink = lightAccent dark.pink;
...
in
{
  ...
  config._module.args = {
    ...
    # serifFont deliberately holds the same family (a fully monospaced desktop);
    ...
    serifFont = "IoskeleyMono Nerd Font";
