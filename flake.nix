{
  ...
  inputs = {
    ...
    # Pinned stable nixpkgs for the slicers. 26.11's orca 2.4.1 regressed file
    # dialogs, and only 26.05 carries bambu-studio's cloud-login fix
    # (nixpkgs#498307: the Bambu network plugin's bundled libcurl needs an
    # explicit CA bundle, and the OAuth webview needs DMA-BUF compositing off).
    # Pinned to an EXACT rev, not the nixos-26.05 branch. The branch gets
    # security backports, and each move rebases the whole slicer closure.
    # Neither slicer build survives that from a cache: bambu-studio is unfree
    # since May 2026 (SFC AGPL ruling, nixpkgs#415821), and the relinked orca
    # (loginFix in modules/apps/orca-slicer.nix) is a custom drv. A branch
    # move therefore costs hours of local compiles. Bump this rev only when
    # you accept that.
    # Intentionally NOT `follows nixpkgs`: a separate pinned closure is the point.
    nixpkgs-2605.url = "github:nixos/nixpkgs/597283ad8aa0b331c788e97c4c262d58877074ef";
